...
const List = ({ searchQueryProp, userId }) => {
  // Get the dark mode context
  const { darkMode } = useContext(DarkModeContext);
  
  // Fetch payment data from the API - filter by userId if provided
  const endpoint = userId ? `/payment/?userId=${userId}` : "/payment/";
  ...
  useEffect(() => {
    if (data) {
      const formatted = data.map(payment => ({
        ...
        postId: payment.booking?.post?.title || "Unknown Property",
